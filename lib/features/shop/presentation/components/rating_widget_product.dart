...
class RatingWidgetProduct extends StatefulWidget {
  final Product product;
  final bool showRatingDialog;
  final double size;
  final Color? color;

  const RatingWidgetProduct({
    super.key,
  ...
  @override
  State<RatingWidgetProduct> createState() => _RatingWidgetProductState();
}

class _RatingWidgetProductState extends State<RatingWidgetProduct> {
  ...
  Future<void> getUserRating() async {
    if (currentUser != null) {
      final rating = await productCubit.getUserRating(
        widget.product.id,
        currentUser!.uid,
      );
      setState(() {
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  selectedRating > 0
                    ...
                    await _rateProduct(selectedRating);
                  }
                      : null,
                  child: Text('Submit'),
                ),
  ...
  Future<void> _rateProduct(int rating) async {
    if (currentUser == null) return;

    try {
      await productCubit.rateProduct(
        widget.product.id,
        currentUser!.uid,
        rating,
      );
  ...
  Widget build(BuildContext context) {
    ...
    final totalRatings = widget.product.userRatings.length;
